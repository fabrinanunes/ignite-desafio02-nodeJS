...
function checksCreateTodosUserAvailability(req, res, next) {
  ...
  if(user.pro === false && user.todos.length >= 10) return res.status(403).json({error: 'This user can not create a TODO'});
...
function checksTodoExists(req, res, next) {
  ...
  if(validate(id) !== true) return res.status(400).json({ error: 'Wrong ID'});
